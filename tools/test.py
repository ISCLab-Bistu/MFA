...
def do_test(
    ...
    if cfg.TEST.RE_RANKING == 'off':
        ...
        evaluator = create_supervised_evaluator(model, metrics={'r1_mAP_mINP': r1_mAP_mINP(num_query, data_loader=data_loader['eval'], max_rank=50, feat_norm=cfg.TEST.FEAT_NORM, vis_rank=cfg.TEST.VISUAL_RANK)},
                                                device=device)
    elif cfg.TEST.RE_RANKING == 'on':
        print("Create evaluator for reranking")
        evaluator = create_supervised_evaluator(model, metrics={'r1_mAP_mINP': r1_mAP_mINP_reranking(num_query, data_loader=data_loader['eval'], max_rank=50, feat_norm=cfg.TEST.FEAT_NORM, vis_rank=cfg.TEST.VISUAL_RANK)},
                                                device=device)
    else:
